         ...
         foreach my $Di (("0.4"))
         {
         	foreach my $line (@Conf)
         	{
         		for(my $Seed = 1; $Seed <=35; $Seed++) ##Realizar 35 ejecuciones con distintas semilla de cada instancia..
         		{
         			...
         			print $fout "~$PathAlgorithm/VSD-MOEA --n 100 --nfes 2500000 --nvar $nvar --Instance $inst --Path $PathAlgorithm --Dist_factor $Di --nobj $nobj --Seed $Seed --param_l 20 --param_k 4 --Last_time_diver $Df --Px $pc --Model $model\n";
         		}
         	}
         }
      }
